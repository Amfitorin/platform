...
namespace MyRI.Anima2D.Scripts.Editor.OnionSkin
{
	public class OnionSkinWindow : EditorWindow
	{
		...
		{
			get {
				if(AnimationWindowExtra.AnimationWindowExtra.activeAnimationClip)
				{
					return (int)(AnimationWindowExtra.AnimationWindowExtra.activeAnimationClip.length * AnimationWindowExtra.AnimationWindowExtra.activeAnimationClip.frameRate);
				}
		...
		{
			get {
				return Mathf.Clamp(AnimationWindowExtra.AnimationWindowExtra.frame,0,frameCount);
			}
		}
		...
		void OnUpdate()
		{
			if(m_EnableOnionSkin)
			{
				...
				{
					if(m_OldClip != AnimationWindowExtra.AnimationWindowExtra.activeAnimationClip)
					{
						CreatePreview();
						UpdatePreview();
					}
					...
					if(AnimationWindowExtra.AnimationWindowExtra.refresh > 0)
					{
						ResamplePreview();
					}
				}
			...
			m_OldClip = AnimationWindowExtra.AnimationWindowExtra.activeAnimationClip;
		...
		void CreatePreview()
		{
			...
			m_InstanceRoot = EditorExtra.InstantiateForAnimatorPreview(AnimationWindowExtra.AnimationWindowExtra.rootGameObject) as GameObject;
		...
		void ResamplePreview()
		{
			...
			m_OnionLayerManager.ResampleOnionLayers(AnimationWindowExtra.AnimationWindowExtra.activeAnimationClip);
		}
		...
		void UpdatePreview()
		{
			...
			m_OnionLayerManager.UpdateOnionLayers(AnimationWindowExtra.AnimationWindowExtra.activeAnimationClip,
			                                      clampedFrame,
			                                      m_NumFrames,
			                                      m_Step,
			                                      m_AlphaMultiplier,
